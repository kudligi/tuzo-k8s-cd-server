...
def create_cryptogen(topo):
    ...
    yaml.dump(crypto_config, open("crypto-config.yaml", "w"))
...
def create_configtx(topo):
    ...
    yaml.dump(configtx, open("configtx.yaml", "w"))

    with open("configtx.yaml", 'r') as f:
       newText=f.read().replace('[]', '')
       open("configtx.yaml", 'w').write(newText)



def create_network(topo):
    Network = {
        "tlsEnabled" : False,
